...
// Error: 'foo' is defined inside the for loop, so it is out of scope on line 10.
//print("The value of foo is \(foo)")
...
// 'x' is defined outside the for loop, so it is accessible and modifiable both inside and outside the loop.
// In contrast, 'foo' was defined inside the loop, so it was not accessible outside.
var x = 10
for _ in 0..<10 {
    x += 1
